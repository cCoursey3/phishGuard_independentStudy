...
for entry in data:
    ...
    labels = entry['labels']  # Retrieve original labels
    ...
    tokenized_input = tokenizer(row_text, return_offsets_mapping=True, truncation=True)
    ...
    for o in offsets:
        start, end = o
        if start == 0 and end == 0:  # Special token (e.g., [CLS], [SEP])
            new_labels.append(0)
            token_idx += 1
        elif start != end:
            if word_idx < len(labels):
                ...
                if whitespace_char in tokens[token_idx]:  # If there is a whitespace we are on another word
                    word_idx += 1
                    if word_idx >= len(labels):
                        break
            else:
